...
####### DESCRIPTION #######
# This script takes the preprocessed images output from slurm_create-data_preproc.ipynb (or preproc_1.py) and makes them cool -- i.e. reshapes and performs ISC
# The purpose of this script is to calculate ISC across all three high and low salience movies, performing LOO on 3 movies.
# The thresh and unthresh ISC are saved in the isc_dir directory.
...
# ## directories 
top_dir = '/jukebox/graziano/coolCatIsaac/MEI' ## CHANGE ME ## 
...
# set empty dic
salience_type = {
     'low' : ['oragami', 'brushing', 'cake'],
    'high': ['sherlock', 'office', 'shrek']
                }
thresh_vis_dic = {cond: {s: {run: {} for run in run_list} for s in list(salience_type.keys())} for cond in cond_list}
nothresh_vis_dic = {cond: {s: {run: {} for run in run_list} for s in list(salience_type.keys())} for cond in cond_list}
...
################### ISC ANAL ################## #
'''
NOTE! THIS SCRIPT CALCULATES ACROSS ALL THREE HIGH OR LOW MOVIES, PERFORMING LOO ON 3 MOVIES.
IN THE PAPER, HOWEVER, I CALCULATE THE ISC FOR EACH MOVIE SEPARATELY AND THEN AVERAGE THE ISC VALUES ACROSS MOVIES.
SUBTLE NUANCE BUT MAKES A DIFFERENCE. YOU CAN ADJUST THIS CODE TO MATCH THE PAPER IF YOU WOULD LIKE.
THIS WOULD INVOLVE CHANGING THE salience_type dictionary to contain only one movie per salience type, AND THEN AVERAGING THE ISC VALUES ACROSS MOVIES.
'''
...
for cond in cond_list:
    ...
    if cond == "external":
        ...
        targ_dic = int_isc
    for run in run_list:
        for s in salience_type:
            print('salience:', s)
            temp_iscs = []
            for mov in salience_type[s]:
                ...
                print('isc...')
                iscs_t = isc(data, pairwise=False, tolerate_nans=.8) # for each movie, one ISC map per subject

                temp_iscs.append(iscs_t)
            #temp_iscs_array = np.array(temp_iscs)
            # Reshape each dataset into (N1 * N3, D2) and concatenate
            reshaped_data = [data.reshape(-1, data.shape[1]) for data in temp_iscs]

            print('SHAPE-temp', np.array(reshaped_data).shape)
            # Concatenate all reshaped datasets
            iscs = np.concatenate(reshaped_data, axis=0) # concatenate ISC maps across movies before taking the mean
            #iscs = temp_iscs_array.reshape(-1, 112179)
            print('SHAPE', iscs.shape)
            ...
            thresh_vis_dic[cond][s][run] = isc_nii
            nothresh_vis_dic[cond][s][run] = isc_nii_nt
            print(f'\nfinish {cond}-{s}-{run}\n')
...
np.save(f'{isc_dir}/{prefix}_isc_sigvox_SAL_nii_dic.npy', thresh_vis_dic)
## save visualization for no threshold
np.save(f'{isc_dir}/{prefix}_isc_noThresh_SAL_nii_dic.npy', nothresh_vis_dic)
